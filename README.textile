...
You can download the latest version "here":https://github.com/mzubala/jquery-custom-scrollbar/blob/master/build/jquert-custom-scrollbar-1.0.zip

h2. Demos

In <pre>demos</pre> folder of this repo, there are some example usages of custom scrollbar and it's api.

h2. Basic Usage

First download and add <pre>jquery.custom-scrollbar.min.js</pre> and <pre>jquery.custom-scrollbar.css</pre> to your site.
...
In the example we use <pre>default-skin</pre>. Plugin comes with two other predefined skins: <pre>gray-skin</pre> and <pre>modern-skin</pre>.
...
Sets horizontal scrollbar position to <pre>x</pre> pixels. <pre>x</pre> should be in range from 0 to scrolled content width. If it's outside that range, content will be scrolled to the start or to the end.
...
Sets vertical scrollbar position to <pre>y</pre> pixels. <pre>x</pre> should be in range from 0 to scrolled content height. If it's outside that range, content will be scrolled to the start or to the end.
